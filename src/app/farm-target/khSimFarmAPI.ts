...
export class FarmClass {
  ...
  private largeCombineText: SVGTextElement;
  ...
  private combineCurrentHarvest: SVGTextElement;
  ...
  private farmTargetText: SVGTextElement
  ...
  public constructor(lowerRenderEl: SVGSVGElement, upperRenderEl: SVGSVGElement, setup: FarmSetup) {
    ...
    this.tensPlotLinesGroup = this.gsvg.getElementById('tens') as SVGSVGElement;
    this.hundredsPlotLinesGroup = this.gsvg.getElementById('hundreds') as SVGSVGElement;
    this.thousandsPlotLinesGroup = this.gsvg.getElementById('thousands') as SVGSVGElement;
    ...
    this.largeCombineText = this.gsvg.getElementById('largeCombineText') as SVGTextElement;
    this.largeCombineTrailer = this.gsvg.getElementById('trailer') as SVGSVGElement
    ...
    this.combineCurrentHarvest = this.gsvg.getElementById('harvestTotalText') as SVGTextElement;
    ...
    this.farmTargetText = this.gsvg.getElementById('farmTargetText') as SVGTextElement
  ...
  private setupInnerLines(index: number): void {
    ...
    for (let j = 1; j < 5; j++) {
      ...
      const innerVerticalLine = document.createElementNS(svgns, 'line');
    ...
    const innerHorizontalLine = document.createElementNS(svgns, 'line');
  ...
  private fillPlot(): void {
    ...
    for (let index = 0; index < 20; index++) {
      for (let jIndex = 50; jIndex > 0; jIndex--) {
        ...
        // animate flower
        const png = document.createElementNS(svgns, 'use');
  ...
  private setAndCenterText(element: SVGTextElement, content: number){
    element.textContent = String(content.toFixed(3))
    gsap.set(element, {x: -element.getBBox().width / 2})
  }
      ...
      this.TL.to(self.largeCombineText, {
        duration: self.harvestDuration, ease: 'linear', onComplete: function() {
          ...
          if (self.cropsHarvested !== postCount) {// update harvest number if off
            ...
            self.setAndCenterText(self.combineCurrentHarvest, self.cropsHarvested)
          }
        }, x: largept.x,
        y: largept.y
      });
  ...
  private reset(): void {
    this.setAndCenterText(this.combineCurrentHarvest, 0)
    this.cropsDeposited = 0;

    this.barnCounterText.textContent = String(0.000)
    ...
    for (let i = 0; i < this.plotArray.length; i++) {
      let element = this.plotArray[i];
      gsap.set(element, {visibility: "hidden", scaleX: 0})
    }
  }
  ...
  private handleDeposit(): void {
    if (!this.animationPlaying && !this.depositAnimating && !this.didUserDrag && this.cropsHarvested > 0) {
      ...
      this.setAndCenterText(this.combineCurrentHarvest, this.cropsHarvested)
      ...
      for (let i = 0; i < Math.ceil(this.cropsHarvested * 10) - 1; i++) {
        const crop = document.createElementNS(svgns, 'use');
      ...
      this.TL.to(crop, {
        duration: 2, onComplete: function() {
          ...
          self.cropsDeposited += parseFloat(self.cropsHarvested.toFixed(3));
          ...
          console.log(self.cropsDeposited, self.setup.target)

          if (self.cropsDeposited > self.setup.target){
            if (confirm("Not quite, try again?")){
              self.reset()
            }
          }
          else if (self.cropsDeposited == self.setup.target){
      ...
      this.trailerCrop = this.gsvg.getElementById('crop' + this.trailerCropID) as SVGSVGElement;
    }
  }
  ...
  private init(): void {
    ...
    // farm plot init
    const farmPlot = document.createElementNS(svgns, 'rect');
    ...
    this.setAndCenterText(this.farmTargetText, this.setup.target)
    ...
    let cropid = 1;
    for (let i = 0; i < 5; i++) {
      let crop = document.createElementNS(svgns, 'use');
      gsap.set(crop, {
      ...
      crop = document.createElementNS(svgns, 'use');
      gsap.set(crop, {
    ...
    this.gsvgu.getElementById('reset').addEventListener('pointerdown', () => this.reset())
  }
}
