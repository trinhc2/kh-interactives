...
export function fractionAPI(_els, _setup) {
    ...
    class fractionCollection {
        id: number
        setup: {"frame": string, "draggable": boolean, "startArray": number[], "lockY": boolean, "lockX": boolean, "controls": string, "maxDenom": number}
        fractionid: string
        fractionGroup: SVGSVGElement
        controlsElement: SVGSVGElement
        ...
        init() {
            ...
            if (this.draggable){
                ...
                this.dragRef = Draggable.create(this.fractionGroup, {
                    ...
                    liveSnap: {
                        x: function(value) {
                            let increment = parseInt(self.setup.snapTo.split(" ")[0])
                            //snap to the closest increment of 20.
                            return Math.round(value / increment) * increment;
                        },
                        y: function(value) {
                            let increment = parseInt(self.setup.snapTo.split(" ")[1])
                            //snap to the closest increment of 20.
                            return Math.round(value / increment) * increment;
                        }
                    }
        ...
        setElementPosition(element: Node, x: number, y:number) {
        ...
        generateFractionCollection() {
            ...
            let controlEl = self.els.getElementById("controls").cloneNode(true)
            let bbox = (self.els.getElementById("controls") as unknown as SVGSVGElement).getBBox()
        ...
        handlePointerDown(e) {
            if (e.target.parentNode == this.fractionGroup || e.target.parentNode.parentNode == this.fractionGroup || e.target.parentNode.parentNode == this.controlsElement) {
