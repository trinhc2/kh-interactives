    ...
    class decomposeClass {
        ...
        generateDecompositionObjects(startingNumber) {
            ...
            for (let i = startingNumber; i > 0;) {
                ...
                gsap.set(decrementText, { attr: { id: 'decrementText' + (decrementNumID), class: "number" }, x: numBBox.x - (numBBox.width / 2) + 50, y: numBBox.y + this.yTextOffset, fontFamily: 'Poppins', textContent: decrementVal.toString(), fontSize: this.textSize, visibility: "hidden", fill: this.color })
                this.drawnElements.appendChild(decrementText)

                let decrementAsset = document.createElementNS(this.svgns, "use") as SVGUseElement
                gsap.set(decrementAsset, { attr: { href: currentAsset, id: 'decrementAsset' + (decrementNumID)}, visibility: "hidden"})
                this.drawnElements.appendChild(decrementAsset)

                let number = document.createElementNS(this.svgns, "text") as SVGTextElement
                gsap.set(number, { attr: { id: 'text' + (i), class: 'number' }, x: numBBox.x - (numBBox.width / 2) + 50, y: numBBox.y + this.yTextOffset, fontFamily: 'Poppins', textContent: i, fontSize: this.textSize, visibility: "hidden"})
            ...
            if (!this.animationFinished && this.numValue > 0) {
                ...
                for (let i = startingNumber; i > decrementVal - 1;) {
                    ...
                    if (assetRowCount[decrementVal] == 3) {//if we have filled row with 3 objects of unique type then proceed to next row
                        ...
                        yVal += (decrementAsset.getBBox().height * this.assetScale) + 10
                        xVal = xOffset
                    }
                    ...
                    if (decrementVal == 1) {
                        this.T.set(decrementAsset, { x: xVal - 42, y: yVal - 38, visibility: "visible" })
                    }
                    else if (decrementVal == 10) {
                        this.T.set(decrementAsset, { x: xVal - 15, y: yVal - 97, visibility: "visible" })
                    }
                    else {
                    ...
                    decrementNumID++;
                    xVal += (decrementAsset.getBBox().width * this.assetScale) + 10
