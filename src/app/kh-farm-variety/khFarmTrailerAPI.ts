...
export class FarmClass {
  ...
  private activeCrop: SVGSVGElement;

  private plantWheat: SVGSVGElement;
  private plantGrape: SVGSVGElement;
  private plantBlue: SVGSVGElement;
  private plantPink: SVGSVGElement;

  public constructor(lowerRenderEl: SVGSVGElement, upperRenderEl: SVGSVGElement) {
    ...
    this.plantWheat = this.findElementUpper('plantWheat')
    this.plantGrape = this.findElementUpper('plantGrape')
    this.plantBlue = this.findElementUpper('plantBlue')
    this.plantPink = this.findElementUpper('plantPink')
    this.activeCrop = this.plantWheat
  ...
  private showPlot(index: number, jIndex: number, timeline: any, delay: string): void {
    timeline.set(this.plotArray[index][jIndex], {attr:{href: this.plotHREF}, visibility: 'visible' }, delay);
    timeline.to(this.plotArray[index][jIndex], { duration: 1, scaleX: 0.22 }, '<');

    timeline.set(this.plotArray[index + 1][jIndex], {attr:{href: this.plotHREF}, visibility: 'visible' }, '<');
      ...
      if (this.plotArray[i][j]) {
        ...
        if (this.gridState === this.bedsButton) { // if working with beds
          if (this.plotArray[i][j].style.visibility === 'hidden') {// if current bed is hidden, unhide it
            gsap.set(this.plotArray[i][j], { attr:{href: this.plotHREF}, visibility: 'visible' });
  ...
  private handleCropChange(element: SVGSVGElement): void {
    let currentPointerStateArr: SVGSVGElement[] = gsap.utils.toArray('circle', this.activeCrop);
    currentPointerStateArr[0].style.fill = '#93c47d';
    this.activeCrop = element;

    if (this.activeCrop == this.plantWheat) {
      this.plotHREF = '#plotWheat';
      this.singleHREF = '#singleWheat';
      this.trailerHREF = '#trailerWheat';
    }
    else if (this.activeCrop == this.plantGrape) {
      this.plotHREF = '#plotGrape';
      this.singleHREF = '#singleGrape';
      this.trailerHREF = '#trailerGrape';
    }
    else if (this.activeCrop == this.plantPink) {
      this.plotHREF = '#pinkFlowerBed';
      this.singleHREF = '#singlePink';
      this.trailerHREF = '#pinkFlowerBed';
    }
    else if (this.activeCrop == this.plantBlue) {
      this.plotHREF = '#blueFlowerBed';
      this.singleHREF = '#singleBlue';
      this.trailerHREF = '#blueFlowerBed';
    }

    currentPointerStateArr = gsap.utils.toArray('circle', this.activeCrop);
  ...
  private init(): void {
    ...
    const plantButtonArr: SVGSVGElement[] = gsap.utils.toArray('circle', this.activeCrop);
    console.log(plantButtonArr, this.activeCrop)
